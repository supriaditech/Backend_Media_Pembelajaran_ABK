...
export class SubMateriProgressService {
  ...
  async createOrUpdateProgress(data: SubMateriProgressDto) {
    ...
    console.log(existingSubMateri);
    const existingMateriProgress = await this.prisma.materi.findUnique({
      where: { id: data.materiProgressId },
    });
    console.log(existingMateriProgress);

    if (!existingUser || !existingSubMateri || !existingMateriProgress) {
    ...
    if (!existingProgress) {
      ...
      const newProgress = await this.prisma.subMateriProgress.create({
        data: {
          userId: data.userId,
          subMateriId: data.subMateriId,
          materiProgressId: data.materiProgressId,
          status: data.status,
